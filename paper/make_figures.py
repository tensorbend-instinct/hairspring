...
steps=[(.015,'Goal + project'),(.205,'Worker loop'),(.395,'Declared checks'),(.585,'Fresh critic'),(.775,'Recorded outcome')]
cols=[(BLUE,BF),(BLUE,BF),(ORANGE,OF),(RED,RF),(GREEN,GF)]
for (x,t),(e,f) in zip(steps,cols): box(ax,x,.765,.165,.12,t,'',e,f,7.3)
...
box(ax,.34,.49,.30,.10,'Canonical event record','typed · one writer · hash-linked · replay',BLUE,BF,7.6)
planes=[(.01,.23,'Recovery',BLUE,BF),(.205,.23,'World',GREEN,GF),(.40,.23,'Memory',ORANGE,OF),(.595,.23,'Accounting',BLUE,BF),(.79,.23,'Evolution',RED,RF)]
for x,y,t,e,f in planes:
 box(ax,x,y,.18,.12,t,'',e,f,7.2); arr(ax,(.49,.48),(x+.09,y+.125),LINE,'--',0)
ax.text(.01,.125,'(c) Authority boundaries',weight='bold',fontsize=9,color=INK)
labels=[('Model','proposes'),('Checker','closes'),('World','admits'),('Scorer','promotes')]
for i,(t,su) in enumerate(labels):
 x=.02+i*.245; box(ax,x,.015,.215,.075,t,su,[BLUE,ORANGE,GREEN,RED][i],[BF,OF,GF,RF][i],7.6)
