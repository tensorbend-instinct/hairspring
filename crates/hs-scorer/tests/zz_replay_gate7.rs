...
fn gate7_proof_1_regression_caught_by_heldout_assay() {
    let tmp = std::path::PathBuf::from("/mnt/instinct-nvme/swbench/replay-check/gate7-case1"); let _ = std::fs::remove_dir_all(&tmp); std::fs::create_dir_all(&tmp).unwrap();
    let (mut scorer, mut lineage) = fresh_lineage(tmp.as_path());
...
fn gate7_proof_1b_promotion_replayable_and_pin_bound() {
    let tmp = std::path::PathBuf::from("/mnt/instinct-nvme/swbench/replay-check/gate7-case2"); let _ = std::fs::remove_dir_all(&tmp); std::fs::create_dir_all(&tmp).unwrap();
    let (mut scorer, _lineage) = fresh_lineage(tmp.as_path());
    let pin = scorer.pin();
...
fn gate7_proof_2_scorer_drift_flagged_by_canaries_before_promotion() {
    let tmp = std::path::PathBuf::from("/mnt/instinct-nvme/swbench/replay-check/gate7-case3"); let _ = std::fs::remove_dir_all(&tmp); std::fs::create_dir_all(&tmp).unwrap();
    let (mut scorer, mut lineage) = fresh_lineage(tmp.as_path());
    let pin = scorer.pin();
...
fn gate7_proof_3_best_of_n_envelope_published() {
    let tmp = std::path::PathBuf::from("/mnt/instinct-nvme/swbench/replay-check/gate7-case4"); let _ = std::fs::remove_dir_all(&tmp); std::fs::create_dir_all(&tmp).unwrap();
    ...
    let cmp = envelope.compare(&candidate, &family, tmp.as_path()).unwrap();
