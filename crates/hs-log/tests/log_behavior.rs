...
fn sid() -> Uuid { Uuid::from_bytes([7; 16]) }

#[test]
fn append_assigns_seq_chain_and_hash() {
    let dir = root();
    let mut w = StreamWriter::create(dir.path(), sid()).unwrap();
    let e0 = w.append(EventBuilder::new(EventKind::ToolCall).payload(Payload::Inline(b"a".to_vec()))).unwrap();
    let e1 = w.append(EventBuilder::new(EventKind::ToolCall).payload(Payload::Inline(b"b".to_vec()))).unwrap();
    assert_eq!(e0.seq, 0);
    assert_eq!(e1.seq, 1);
...
fn clean_resume_continues_chain_without_rewrite() {
    ...
    let mut w = StreamWriter::create(dir.path(), sid()).unwrap();
    for i in 0..5u8 {
        w.append(EventBuilder::new(EventKind::ToolCall).payload(Payload::Inline(vec![i]))).unwrap();
    }
    let tail_hash = w.last_hash();
    drop(w);
    let before = StreamReader::open(dir.path(), sid()).unwrap().events().unwrap();
    ...
    let after = StreamReader::open(dir.path(), sid()).unwrap().events().unwrap();
...
fn torn_tail_from_kill_is_truncated_on_resume() {
    ...
    let seg = dir.path().join("streams").join(sid().to_string()).join("seg-000000.hslog");
    use std::io::Write;
...
fn large_payload_is_stored_by_hash_and_reads_back() {
    ...
    let mut w = StreamWriter::create(dir.path(), sid()).unwrap();
    let e = w.append(EventBuilder::new(EventKind::Observation).payload(Payload::Inline(big.clone()))).unwrap();
    match &e.payload {
...
fn deleted_middle_event_breaks_the_chain() {
    ...
    let err = verify_stream(dir.path(), sid()).unwrap_err();
    assert!(
        matches!(err.kind, CorruptionKind::SeqGap { .. } | CorruptionKind::PrevHashMismatch { .. }),
        "expected gap or chain break, got {:?}", err.kind
    );
}
...
fn deleted_and_bitflipped_blobs_are_caught() {
    ...
    let mut w = StreamWriter::create(dir.path(), sid()).unwrap();
    let e = w.append(EventBuilder::new(EventKind::Observation).payload(Payload::Inline(big))).unwrap();
    let Payload::BlobRef { hash, .. } = e.payload else { panic!() };
    drop(w);
    let blob = blob_path(dir.path(), &hash);
    std::fs::remove_file(&blob).unwrap();
    let err = verify_stream(dir.path(), sid()).unwrap_err();
    assert!(matches!(err.kind, CorruptionKind::BlobMissing { .. }), "{:?}", err.kind);

    let dir2 = root();
    let mut w2 = StreamWriter::create(dir2.path(), sid()).unwrap();
    let e2 = w2.append(EventBuilder::new(EventKind::Observation).payload(Payload::Inline(vec![9u8; INLINE_CAP + 1]))).unwrap();
    let Payload::BlobRef { hash: h2, .. } = e2.payload else { panic!() };
    drop(w2);
    ...
    assert!(matches!(err.kind, CorruptionKind::BlobHashMismatch { .. }), "{:?}", err.kind);
}
