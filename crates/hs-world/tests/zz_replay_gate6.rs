...
fn gate6_proof_shared_world_executable_inheritance() {
    let root = std::path::PathBuf::from("/mnt/instinct-nvme/swbench/replay-check/gate6-case1"); let _ = std::fs::remove_dir_all(&root); std::fs::create_dir_all(&root).unwrap();
    let log_root = root.as_path().join("log");
