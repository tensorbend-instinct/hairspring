...
fn realistic_ws(root: &Path) {
    ...
    std::fs::write(
        ws.join("venv/lib/python3.12/site-packages/dep.py"),
        "import os\n",
    )
    .unwrap();
    std::fs::write(
...
fn tree_diff(a: &Path, b: &Path) -> Vec<String> {
    ...
    while let Some((pa, pb)) = stack.pop() {
        ...
        for name in ea {
            let fa = pa.join(&name);
            let fb = pb.join(&name);
            if std::fs::metadata(&fa).unwrap().is_dir() {
                stack.push((fa, fb));
...
fn snapshot_restore_is_byte_exact_and_hash_verified() {
    ...
    assert!(
        rep.files >= 6 && rep.bytes > 4096,
        "full tree captured: {rep:?}"
    );
    ...
    let rrep = world
        .restore(&rep.snapshot_id, &dir.path().join("ws"))
        .unwrap();
    assert!(
        rrep.took_ms < 60_000,
        "tier-B budget is < 60 s warm; measured {} ms",
        rrep.took_ms
    );
    let diffs = tree_diff(&ref_root.join("ws"), &dir.path().join("ws"));
    assert!(
        diffs.is_empty(),
        "restored tree must be byte-identical: {diffs:?}"
    );
}
...
fn snapshot_lands_on_the_log_and_tampering_fails() {
    ...
    let hit = events.iter().any(|e| {
        e.kind == hs_core::EventKind::SnapshotRef
            && String::from_utf8_lossy(&reader.resolve_payload(e).unwrap())
                .contains(&rep.snapshot_id)
    });
...
fn snapshot_preserves_symlinks_as_symlinks() {
    ...
    assert!(
        ...
        "python must be restored as a symlink"
    );
    assert_eq!(
        std::fs::read_link(ws.join("venv/bin/python"))
            .unwrap()
            .to_string_lossy(),
        "python3.12"
    );
    assert_eq!(
        std::fs::read_link(ws.join("venv/dangling"))
            .unwrap()
            .to_string_lossy(),
        "/nonexistent-target-xyz"
    );
    assert_eq!(
        std::fs::read(ws.join("venv/bin/python3.12")).unwrap(),
        b"#!/bin/sh\n"
    );
}
