...
fn main() {
    let script: Vec<String> = std::fs::read_to_string(
        std::env::var("HS_VF_SCRIPT").expect("HS_VF_SCRIPT"),
    )
        ...
        "model.call" => {
            ...
            if prompt.contains("ADVERSARIAL VERIFIER") {
                ...
                let token = prompt
                    ...
                    .and_then(|o| o.trim().strip_prefix("task-").and_then(|n| n.parse::<usize>().ok()).map(|n| format!("TOKEN-{n}-SECRET")));
