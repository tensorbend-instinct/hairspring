...
3. {{\"tool\":\"repo.exec\",\"args\":{{\"command\":\"<any command>\",\"path\":\"<ANSWER_PATH>\"}}}} - run lint/tests on YOUR current patch inside a sandbox (applied to a scratch copy; the repo stays clean; no network, no host fs). If the patch does not apply you get the git error back free - fix the framing before spending a checker cycle. Run the FAIL_TO_PASS command before every answer.write.\n\
