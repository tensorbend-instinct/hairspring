...
"#
    ...
    let ledger = std::fs::read_to_string(log.path().join("ledger.txt")).unwrap();
    assert!(
        ledger.contains("hello-via-mcp"),
        "the mcp echo round-trips through the loop:\n{ledger}"
    );
    assert!(
        ledger.contains("mcp.fixture.echo"),
        "the prompt/advertised catalog names the tool:\n{ledger}"
    );
}
