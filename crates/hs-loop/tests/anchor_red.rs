...
fn anchored_read_renders_line_hash_prefixes() {
    ...
    assert!(text.contains("1:") && text.contains("2:"), "line numbers: {text:?}");
...
fn anchor_replace_applies_and_returns_fresh_anchors_and_cumulative() {
    ...
    let v = editapply::apply_anchor_edits(&ws, "src/main.py", json!([
        {"op": "replace", "anchor": anchor, "content": "    return 2"}
    ]));
    assert!(v.get("$error").is_none(), "apply: {v}");
    assert!(v["snippet"].as_str().unwrap_or("").contains("return 2"), "fresh anchors back: {v}");
...
fn anchor_stale_or_wrong_named_error_candidate_untouched() {
    let ws = fixture_ws();
    let _ = editapply::apply_anchor_edits(&ws, "src/main.py", json!([
        {"op": "replace", "anchor": "2:zzzz", "content": "x"}
    ]));
    let before = editapply::cumulative_diff(&ws)["cumulative_diff"].as_str().unwrap_or("").to_string();
    let v = editapply::apply_anchor_edits(&ws, "src/main.py", json!([
        {"op": "replace", "anchor": "2:zzz:zzz", "content": "    return 9"}
    ]));
    assert!(v.get("$error").is_some(), "bad anchor must error: {v}");
    let after = editapply::cumulative_diff(&ws)["cumulative_diff"].as_str().unwrap_or("").to_string();
...
fn anchor_insert_after_and_write_ops() {
    ...
    assert!(v["cumulative_diff"].as_str().unwrap().contains("+    # done"), "{v}");
    let v2 = editapply::apply_anchor_edits(&ws, "src/new.py", json!([
        {"op": "write", "content": "x = 1\n"}
    ]));
    assert!(v2.get("$error").is_none(), "write new file: {v2}");
    assert!(v2["cumulative_diff"].as_str().unwrap().contains("+x = 1"), "{v2}");
}

#[test]
fn anchor_rejects_escaping_paths() {
    let ws = fixture_ws();
    let v = editapply::apply_anchor_edits(&ws, "../evil.txt", json!([
        {"op": "write", "content": "x"}
    ]));
...
fn anchor_edits_then_submit_git_apply_clean() {
    ...
    let pristine = std::env::temp_dir().join(format!("{}-pristine", ws.file_name().unwrap().to_string_lossy()));
    let o = Command::new("git").args(["clone", "-q"]).arg(&ws).arg(&pristine).output().unwrap();
    assert!(o.status.success());
    let chk = Command::new("git").args(["apply", "--check"]).arg(&answer).current_dir(&pristine).output().unwrap();
    assert!(chk.status.success(), "anchor-path submit must git-apply clean: {}", String::from_utf8_lossy(&chk.stderr));
}
