...
fn empty_home_centers_identity_and_prompt_instead_of_showing_an_empty_dashboard() {
    ...
    assert!(s.contains("What are we building?"), "single central prompt: {s}");
    assert!(s.contains("DeepSeek V4 Pro"), "model next to prompt: {s}");
    assert!(s.contains("~/hairspring"), "quiet cwd footer: {s}");
    assert!(!s.contains("OBSERVE › ACT › CHECK › REFLECT"), "idle machinery must not dominate home: {s}");
}
...
fn active_session_has_one_header_message_canvas_prompt_and_contextual_footer() {
    ...
    assert!(s.contains("HAIRSPRING: Fix the verifier"), "clear session header: {s}");
    assert!(s.contains("abc123"), "session id at header edge: {s}");
    assert!(s.contains("Find and fix the bug"), "message canvas: {s}");
    assert!(s.contains("Queue a follow-up"), "prompt says what Enter does while active: {s}");
    assert!(s.contains("73% context"), "model/context meter: {s}");
    assert!(s.contains("enter queue") && s.contains("esc interrupt"), "state-aware shortcuts: {s}");
...
fn tool_activity_stays_inline_and_never_opens_a_permanent_machine_dashboard() {
    ...
    st.on_ui_event(&UiEvent::ToolCallStart { plugin: "repo.search".into(), args_summary: "verifier".into() });
    let s = screen(&st, 90, 26);
    assert!(s.contains("▣") && s.contains("repo.search"), "inline tool row: {s}");
    assert!(!s.contains("T_mission") && !s.contains("lineage") && !s.contains("scorer"), "advanced machinery remains on-demand: {s}");
}
