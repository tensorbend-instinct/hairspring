...
fn edit_patch(ws: &PathBuf, patch: &str) -> serde_json::Value {
...
fn edit_patch_update_applies_to_candidate_and_returns_cumulative_diff() {
    ...
    assert!(cand_file.contains("return 1"), "live ws must never be touched");
}
...
fn edit_patch_rejects_escaping_paths() {
    ...
    for bad in ["../evil.txt", "/etc/passwd"] {
        let v = edit_patch(&ws, &format!("*** Begin Patch\n*** Add File: {bad}\n+x\n*** End Patch\n"));
        assert!(v["$error"].as_str().unwrap_or("").contains("path"), "escape must be rejected: {bad} -> {v}");
    }
}
...
fn answer_submit_writes_git_apply_clean_diff_including_new_files() {
    ...
    assert!(text.starts_with("diff --git"), "pure git diff, no fence/prose: {:?}", &text[..80.min(text.len())]);
    ...
    assert!(chk.status.success(), "submit output must git-apply clean: {}", String::from_utf8_lossy(&chk.stderr));
}
