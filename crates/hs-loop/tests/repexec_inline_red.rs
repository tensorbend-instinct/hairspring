...
const RAW_DIFF: &str = "diff --git a/app.py b/app.py\n--- a/app.py\n+++ b/app.py\n@@ -1 +1 @@\n-x = 1\n+x = 2\n";
...
fn inline_diff_accepts_fenced_or_raw() {
    ...
    assert!(r["stdout"].as_str().unwrap().contains("x = 2"), "fenced: {r}");
}
...
fn inline_diff_bad_patch_is_clean_feedback_not_machinery_error() {
    ...
    assert!(r.get("$error").is_none(), "machinery must not fail on model input: {r}");
    assert!(r.get("note").is_some() || r.get("apply_error").is_some(), "feedback: {r}");
}
...
fn serve_roundtrip(bin: &str, ws: &std::path::Path, req: &str) -> serde_json::Value {
    ...
    std::thread::spawn(move || { stdin.write_all(req.as_bytes()).unwrap(); drop(stdin); });
    let out = p.wait_with_output().unwrap();
    let line = String::from_utf8_lossy(&out.stdout);
    let resp = line.lines().nth(1).unwrap_or_else(|| panic!("describe + call responses: {line}"));
...
fn plugin_inline_diff_beats_stale_answer_path_when_both_given() {
    ...
    let r = &resp["result"];
    assert_eq!(r["applied"], true, "{resp}");
    assert!(r["stdout"].as_str().unwrap().contains("x = 2"), "inline diff must win: {resp}");
    assert!(!r["stdout"].as_str().unwrap().contains("x = 99"), "stale answer must not shadow: {resp}");
}
