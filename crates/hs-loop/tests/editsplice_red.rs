...
fn blk(path: &str, old: &str, new: &str) -> hs_loop::editapply::EditBlock {
    hs_loop::editapply::EditBlock { path: path.into(), old: old.into(), new: new.into() }
}
...
fn splice_accumulates_across_calls_and_survives_reentry() {
    ...
    let r2 = hs_loop::editapply::apply_blocks(d.path(), &[blk("lib.py", "    return 1\n", "    return 2\n")]);
    ...
    hs_loop::editapply::reset(d.path());
    assert_eq!(hs_loop::editapply::cumulative_diff(d.path())["has_candidate"], false);
}
...
fn splice_old_not_found_is_clean_feedback_and_candidate_unchanged() {
    ...
    let r1 = hs_loop::editapply::apply_blocks(d.path(), &[blk("app.py", "x = 1\n", "x = 2\n")]);
    assert_eq!(r1["applied"], true, "{r1}");
    let r2 = hs_loop::editapply::apply_blocks(d.path(), &[blk("lib.py", "    return 99\n", "    return 2\n")]);
...
fn splice_multi_file_one_call_all_or_nothing() {
    let d = mk_ws();
    let r = hs_loop::editapply::apply_blocks(d.path(), &[
        blk("app.py", "x = 1\n", "x = 2\n"),
        blk("lib.py", "    return 1\n", "    return 2\n"),
    ]);
    assert_eq!(r["applied"], true, "{r}");
    let cd = r["cumulative_diff"].as_str().unwrap();
    assert!(cd.contains("+x = 2") && cd.contains("+    return 2"), "{cd}");
    // one bad block anywhere fails the whole call and writes nothing
    let d2 = mk_ws();
    let r2 = hs_loop::editapply::apply_blocks(d2.path(), &[
        blk("app.py", "x = 1\n", "x = 2\n"),
        blk("lib.py", "    return 99\n", "    return 2\n"),
    ]);
    assert_eq!(r2["applied"], false, "{r2}");
    assert_eq!(hs_loop::editapply::cumulative_diff(d2.path())["has_candidate"], true);
    assert!(!hs_loop::editapply::cumulative_diff(d2.path())["cumulative_diff"].as_str().unwrap().contains("+x = 2"),
        "no partial writes");
    hs_loop::editapply::reset(d.path());
...
fn plugin_applies_edits_blocks() {
    ...
    assert!(r[0]["result"]["cumulative_diff"].as_str().unwrap().contains("+x = 2"), "{r:?}");
    hs_loop::editapply::reset(d.path());
}
...
fn plugin_retires_raw_diff_with_steering_error() {
    ...
    assert!(!std::fs::read_to_string(d.path().join("app.py")).unwrap().contains("x = 2"));
    hs_loop::editapply::reset(d.path());
}
