...
fn events_tell_the_two_arms_apart() {
    ...
    rig(dir.path(), log_on.path(), true)
        .run_mission("task-0")
        .unwrap();
    rig(dir.path(), log_off.path(), false)
        .run_mission("task-0")
        .unwrap();
    ...
    for (name, evs) in [("on", &on), ("off", &off)] {
        assert!(
            evs.iter().any(|e| e.kind == hs_core::EventKind::Feedback),
            "{name}: checker verdicts must be recorded in BOTH arms"
        );
    }
    assert!(
        on.iter()
            .any(|e| e.kind == hs_core::EventKind::ContextInject),
...
fn events(log: &std::path::Path) -> Vec<hs_core::Event> {
    ...
    hs_log::StreamReader::open(log, sid)
        .unwrap()
        .events()
        .unwrap()
}
fn only_stream(log: &std::path::Path) -> uuid::Uuid {
    let mut v: Vec<_> = std::fs::read_dir(log.join("streams"))
        .unwrap()
