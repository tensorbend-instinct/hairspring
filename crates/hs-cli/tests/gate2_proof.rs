...
impl Driver {
    fn start(dir: &std::path::Path, config: &std::path::Path) -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_hs-gate2-driver"))
            .args(["--dir", dir.to_str().unwrap(), "--config", config.to_str().unwrap()])
...
"#)).unwrap();
    ...
    assert!(d.cmd("model fake-v2 second model").starts_with("OK fake-v2:"));
    assert!(d.cmd("tool echo still here").starts_with("OK echo:"));
    assert_eq!(d.pid, pid_before, "harness process was restarted: gate falsified");

    // And every call - before and after the change - is on the canonical log.
    drop(d);
    let mut entries: Vec<_> = std::fs::read_dir(dir.path().join("streams")).unwrap()
        .map(|e| e.unwrap().file_name().into_string().unwrap()).collect();
    assert_eq!(entries.len(), 1);
    let sid = uuid::Uuid::parse_str(&entries.pop().unwrap()).unwrap();
    let events = hs_log::StreamReader::open(dir.path(), sid).unwrap().events().unwrap();
    let kinds: Vec<_> = events.iter().map(|e| e.kind).collect();
    assert!(kinds.iter().filter(|k| **k == hs_core::EventKind::ToolCall).count() >= 3);
    assert!(kinds.iter().filter(|k| **k == hs_core::EventKind::ModelCall).count() >= 2);
    let joined = events.iter().map(|e| format!("{:?}", e.payload)).collect::<String>();
    assert!(joined.contains("HELLO") && joined.contains("fake-v2"), "phase-2 calls not on the log");
    ...
    println!("  {} events on the canonical log, chain verified", events.len());
}
