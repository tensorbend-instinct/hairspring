...
fn run_demo(dir: &std::path::Path, args: &[&str]) -> std::process::Output {
    let mut full = vec!["--dir", dir.to_str().unwrap(), "--steps", "60", "--seed", "42"];
...
fn final_hash_of(out: &std::process::Output) -> String {
    ...
    s.lines().find_map(|l| l.strip_prefix("FINAL ")).unwrap_or_else(|| panic!("no FINAL in: {s}\nstderr: {}", String::from_utf8_lossy(&out.stderr))).to_string()
}

fn stream_of(dir: &std::path::Path) -> uuid::Uuid {
    let mut entries: Vec<_> = std::fs::read_dir(dir.join("streams")).unwrap().map(|e| e.unwrap().file_name().into_string().unwrap()).collect();
...
fn gate1_proof_1_kill_mid_run_resume_zero_loss() {
    ...
    let ref_events = StreamReader::open(refdir.path(), ref_sid).unwrap().events().unwrap();
    assert_eq!(ref_final, hex(&oracle_state(SEED, STEPS)), "demo disagrees with oracle");

    // Victim: same run, killed with SIGKILL partway through.
    let dir = tempfile::tempdir().unwrap();
    let mut child = Command::new(env!("CARGO_BIN_EXE_hs-demo"))
        .args(["--dir", dir.path().to_str().unwrap(), "--steps", "60", "--seed", "42",
               "run", "--step-delay-ms", "40"])
    ...
    let pre_resume_events = StreamReader::open(dir.path(), sid).unwrap().events().unwrap();
    assert!(pre_resume_events.len() < ref_events.len(), "victim finished before the kill: test invalid");
    assert!(pre_resume_events.len() >= 5, "kill landed too early: test invalid");
    ...
    let post = StreamReader::open(dir.path(), sid).unwrap().events().unwrap();
    assert_eq!(&post[..pre_resume_events.len()], &pre_resume_events[..], "resume rewrote history");
    ...
    // (e) tier-A budget
    assert!(resume_wall < Duration::from_secs(5), "resume took {:?}, over tier-A budget", resume_wall);

    println!("PROOF-1 kill-mid-run resume: PASS");
    println!("  killed after seq {killed_at_seq} of {} events; resume wall time {:?} (< 5 s tier-A)",
             ref_events.len(), resume_wall);
    println!("  final state {} == uninterrupted reference == oracle", resumed_final);
}
...
fn gate1_proof_2_rewind_and_replay_to_identical_state() {
    ...
    let mut checked = 0;
    for e in &events {
        if e.kind != EventKind::Decision { continue; }
        let Payload::Inline(body) = &e.payload else { panic!("breakpoint payload not inline") };
        ...
        assert_eq!(rebuilt, Some((step, recorded)), "replay to seq {} diverged", e.seq);
        assert_eq!(recorded, oracle_state(SEED, step), "breakpoint state wrong at step {step}");
        checked += 1;
    }
    assert!(checked >= 5, "expected breakpoints to check, got {checked}");
    println!("PROOF-2 rewind/replay: PASS - {checked} breakpoints replayed to oracle-identical state");
}
...
fn hs_demo_state_from_events(events: &[Event]) -> Option<(u64, [u8; 32])> {
    events.iter().rev().find_map(|e| {
        if e.kind != EventKind::ToolCall { return None; }
        let Payload::Inline(b) = &e.payload else { return None };
        if b.len() < 40 { return None; }
        Some((u64::from_le_bytes(b[0..8].try_into().unwrap()), b[8..40].try_into().unwrap()))
    })
}
...
fn gate1_proof_3_corrupted_byte_is_caught() {
    ...
    println!("PROOF-3 corruption caught: PASS - flipped byte reported at seq {} ({:?})", err.seq, err.kind);
}
