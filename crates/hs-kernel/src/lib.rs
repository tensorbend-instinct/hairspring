...
/// A standalone caller for one model plugin, ownable by a worker thread
/// (gate-8 async verifier: the audit call runs off the mission thread
/// while the agent keeps working). Built from the SAME config entry the
/// kernel uses, so command, lease, and supervisor semantics are identical
/// to a synchronous call; describe-validation already happened at
/// Kernel::load.
...
impl ModelCaller {
    ...
    /// One model.call round trip with the same outcome mapping as
    /// Kernel::call_model_with.
    pub fn call(
        &mut self,
        prompt: &str,
        tools: Option<&serde_json::Value>,
        ...
        Ok(ModelOutcome {
            ...
            cost_usd_micros: r["cost_usd_micros"].as_i64().unwrap_or(0),
            latency_ms,
            model: self.slot.entry.name.clone(),
        })
    }
}
