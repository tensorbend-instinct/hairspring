...
while true; do
  ...
  OUT=$(tools google-drive update --file-id "$FILE_ID" --file-path /tmp/mirror.txt --mime-type text/plain 2>&1) \
    || echo "$(date '+%F %T') update failed: $(echo "$OUT" | head -c 200)" >> /tmp/swe-mirror.log
